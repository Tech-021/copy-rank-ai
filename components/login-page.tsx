...
export function LoginPage({ onLoginSuccess, onBackToLanding, onToggleSignUp }: LoginPageProps) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    // Optional: onLoginSuccess can be called here if your Google login returns email
  }
    ...
    <div className="min-h-screen bg-white flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-blue-500 to-blue-600 rounded-3xl m-4 p-12 flex-col justify-between relative overflow-hidden">
        ...
        <div className="relative z-10 flex items-center justify-center gap-2 mt-12">
          <button onClick={handlePrevTestimonial} className="text-white hover:text-blue-100 transition">
            ●
          </button>
          {testimonials.map((_, index) => (
            <button
              ...
              className={`h-1 rounded-full transition-all ${
                index === currentTestimonial ? "bg-white w-8" : "bg-blue-300 w-2"
              }`}
            />
          ))}
          <button onClick={handleNextTestimonial} className="text-white hover:text-blue-100 transition">
            ●
          </button>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-between p-8 lg:p-16">
        ...
        <div>
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              <label className="block text-gray-700 font-medium mb-2">Work email address</label>
              <input
                type="email"
                value={email}
            ...
            <label className="flex items-center gap-2 cursor-pointer">
              <div className="relative">
                ...
                />
                {rememberMe && (
                  <Check size={16} className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-white pointer-events-none" />
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="border border-gray-300 rounded-lg py-3 px-4 flex items-center justify-center gap-2 hover:bg-gray-50 transition"
              >
                Google
              </button>
            </div>
            ...
            <p className="text-gray-600">
              ...
              <button onClick={onToggleSignUp} className="text-blue-500 hover:text-blue-600 font-medium">
                Sign Up
              </button>
            </p>
          </form>
